...
def get_latest_version(app_name: str, config: dict) -> str | None:
    ...
    for uptodown_name in possible_names:
        ...
        try:
            ...
            logging.debug(f"Failed for {url}: {e}")
            continue
...
def get_download_link(version: str, app_name: str, config: dict) -> str | None:
    ...
    logging.info(
        f"Searching {len(possible_names)} possible Uptodown names for {app_name} v{version}"
    )
    ...
    for uptodown_name in possible_names:
        ...
        try:
            ...
            logging.debug(f"Pattern {uptodown_name} failed: {e}")
            continue
